...
@if(\Illuminate\Support\Facades\Request::is('/'))
    <meta name="description" content="{{ $settings->where('name', 'home_meta_description')->first()->value }}">
    <meta name="robots" content="index, follow">
    <meta name="keywords" content="{{ $settings->where('name', 'home_meta_keywords')->first()->value }}">
    <link rel="canonical" href="{{ route('Public > Home') }}">

    <meta property="og:locale" content="{{ app()->currentLocale() }}">
    <meta property="og:title" content="{{ trim(View::getSection('page_title')) }}">
    <meta property="og:site_name" content="{{ env('APP_NAME') }}">
    <meta property="og:description" content="{{ $settings->where('name', 'home_meta_description')->first()->value }}">
    <meta property="og:type" content="website">
    <meta property="og:url" content="{{ route('Public > Home') }}">

    <meta name="twitter:site" content="{{ route('Public > Home') }}" />
    <meta name="twitter:title" content="{{ trim(View::getSection('page_title')) }}" />
    <meta name="twitter:description" content="{{ $settings->where('name', 'home_meta_description')->first()->value }}" />
@endif
{{--Advertisements--}}
            ...
            "openingHours": [
                {
                  ...
                  "dayOfWeek": [
                    @foreach ($workingDays as $wd) "{{ $wd }}"@if(!$loop->last),@endif @endforeach
                  ],
